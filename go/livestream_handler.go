...
func reserveLivestreamHandler(c echo.Context) error {
	...
	startTimes := make([]int64, len(slots))
	endTimes := make([]int64, len(slots))
	for i, slot := range slots {
		startTimes[i] = slot.StartAt
		endTimes[i] = slot.EndAt
	}

	var counts []int
	query, args, err := sqlx.In("SELECT slot FROM reservation_slots WHERE start_at IN (?) AND end_at IN (?)", startTimes, endTimes)
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to get reservation_slots: "+err.Error())
	}
	...
	// タグ追加
	if len(req.Tags) > 0 {
		livestreamTags := make([]LivestreamTagModel, len(req.Tags))
		for i, tagID := range req.Tags {
			livestreamTags[i] = LivestreamTagModel{
				LivestreamID: livestreamID,
				TagID:        tagID,
			}
		}
		// Bullinsert
		if _, err = tx.NamedExecContext(ctx, "INSERT INTO livestream_tags (livestream_id, tag_id) VALUES (:livestream_id, :tag_id)", livestreamTags); err != nil {
...
func fillLivestreamResponse(ctx context.Context, tx *sqlx.Tx, livestreamModel LivestreamModel) (Livestream, error) {
	...
	`
	if err := tx.SelectContext(ctx, &tags, query, livestreamModel.ID); err != nil {
		return Livestream{}, err
	}
